...
namespace Jenetics.Example
{
    public class Knapsack : IProblem<IImmutableSeq<Knapsack.Item>, BitGene, double>
    {
        ...
        private static Knapsack Of(int itemCount, Random random)
        {
            ...
            items.Select( item => Item.Random( random ) );

            return new Knapsack(items.ToImmutableSeq(), itemCount * 100.0 / 3.0);
        }
        ...
        public class Item
        {
            ...
            {
                return obj is Item &&
                       Comparer.Default.Compare(_size, (obj as Item)._size) == 0 &&
                       Comparer.Default.Compare(_value, (obj as Item)._value) == 0;
            }
