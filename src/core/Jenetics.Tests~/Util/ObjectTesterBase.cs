        ...
        where T : class
    {
        ...
        protected IMutableSeq<T> NewEqualObjects(int length)
        {
            ...
            for (var i = 0; i < length; i++)
                items.Add( With(new Random(589), r => Factory()()) );
            
            return items.ToMutableSeq();
        }
        ...
        {
            var that = Factory()();
            if (that is ICopyable<T>)
            {
                var other = (that as ICopyable<T>).Copy();
                if (other.GetType() == (that as ICopyable<T>).GetType())
                {
                    Assert.Equal(that, other);
                    Assert.NotSame((that as ICopyable<T>), other);
                }
            }
        }
        ...
        public virtual void IsValid()
        {
            var a = Factory()();
            if (a is IVerifiable)
                Assert.True((a as IVerifiable).IsValid);
        }
